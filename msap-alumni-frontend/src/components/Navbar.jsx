...
export default function Navbar() {
  ...
  return (
    <>
      <nav className="glass-nav text-ink sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-5">
          <div className="flex justify-between items-center h-18">
                ...
                >
                  <button
                    className={`text-[14px] font-semibold px-4 py-2 rounded-xl transition-all flex items-center gap-1.5 cursor-pointer ${
                      activeMega === idx
                        ? 'text-lavender bg-lavender-soft/80 shadow-2xs font-bold'
                        : 'text-stone hover:text-lavender hover:bg-parchment-subtle/80'
                    }`}
                  >
                    {item.label}
                    <svg
                  ...
                  {/* Mega Menu Dropdown */}
                  {activeMega === idx && (
                    <div className="absolute top-[90%] left-1/2 -translate-x-1/2 w-[520px] bg-white/95 backdrop-blur-xl border border-lavender/25 shadow-[0_20px_50px_rgba(28,20,46,0.14)] rounded-3xl overflow-hidden z-50 animate-heroIn">
                      <div className="grid grid-cols-2 gap-0 p-6">
                        {item.children.map((group, gi) => (
                          <div key={gi} className={gi > 0 ? 'pl-6 border-l border-parchment-dark/70' : ''}>
                            ...
                            <ul className="space-y-1">
                              {group.links.map((link, li) => (
                                <li key={li}>
                                  <Link
                                    to={link.to}
                                    className="group/item flex items-center gap-2.5 text-[13.5px] font-medium text-stone hover:text-lavender p-2 rounded-xl hover:bg-lavender-soft/50 transition-all"
                                  >
                      ...
                      <div className="bg-lavender-subtle/50 px-6 py-2.5 border-t border-parchment-dark/70 flex items-center justify-between text-[12px] text-muted font-medium">
            ...
            <div className="hidden lg:flex items-center gap-3.5">
              {currentUser ? (
                <div className="flex items-center gap-3">
                  <Link
                    to="/community"
                    className="group flex items-center gap-2.5 px-3.5 py-1.5 rounded-full bg-white border border-lavender/30 text-ink text-[13px] font-bold shadow-2xs hover:border-lavender transition-all"
                  >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Link
                    to="/register"
                    className="btn-hover text-[14px] font-bold text-white bg-lavender hover:bg-lavender-dark px-5 py-2.5 rounded-xl shadow-[0_6px_20px_rgba(88,59,156,0.25)] transition-all"
                  >
                    Register Now
                  </Link>
                </div>
            ...
            <button
              className="lg:hidden text-ink hover:text-lavender p-2 rounded-xl bg-parchment-subtle border border-parchment-dark"
        ...
        {/* Mobile menu drawer */}
        {mobileOpen && (
          <div className="lg:hidden bg-white/95 backdrop-blur-xl border-t border-parchment-dark max-h-[80vh] overflow-y-auto shadow-2xl animate-heroIn">
            <div className="px-5 py-5 space-y-2">
              {NAV_ITEMS.map((item, idx) => (
                <div key={idx} className="border-b border-parchment-subtle pb-2">
                  <button
              ...
              <div className="pt-4 space-y-2.5">
                {currentUser ? (
                  <div className="space-y-2">
                    <div className="p-3 bg-lavender-soft/70 rounded-xl border border-lavender/25 flex items-center gap-2 text-xs font-bold text-ink">
                      <span className="w-2.5 h-2.5 rounded-full bg-emerald-600 inline-block" />
                      Signed in as {currentUser.fullName}
                    </div>
                    <button
                      onClick={handleSignOut}
                      className="w-full text-center text-sm font-bold text-stone border border-parchment-dark py-2.5 rounded-xl hover:text-vermilion"
                    >
                      Sign Out
                    </button>
                  </div>
                  ...
                  <>
                    <Link
                      to="/login"
                      className="block w-full text-center text-sm font-bold text-stone border-2 border-parchment-dark py-3 rounded-xl hover:border-lavender hover:text-lavender transition-colors"
                    >
                      Sign In
                    </Link>
                    <Link
