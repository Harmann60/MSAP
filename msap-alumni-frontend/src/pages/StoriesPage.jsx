...
export default function StoriesPage() {
  ...
  useEffect(() => {
    fetchStories()
      ...
      .catch((err) => {
        console.warn('Using offline stories data:', err.message);
      });
  }, []);

  const featured = stories[0] || DEFAULT_STORIES[0];
        ...
        <Link to="/stories" className="group block mb-14">
          <div className="md:flex">
            ...
            <div className="md:w-2/5 p-6 md:p-8 flex flex-col justify-center">
              <div className="text-[11px] font-semibold uppercase tracking-widest text-muted mb-3">{featured.source} &middot; {featured.date}</div>
              <h2 className="font-display text-ink text-2xl md:text-3xl mb-3 group-hover:text-vermilion transition-colors">{featured.title}</h2>
